...
out = foreach(t = (winSize+1):nrow(wDF), .packages = c("DEoptimR"), .combine = rbind) %dopar% {
  try({
    ...
    set.seed(1234)
    theta <- JDEoptim(rep(-1, nCat), rep(1, nCat), fn=meanUOptim, constr=consLasso, maxiter=200)# , trace = TRUE, triter = 1) 
    return(theta$par)
  })
}
stopCluster(cl)

write.csv(file="outLassoFinal.csv", x=out, row.names=FALSE)
...
#### EDIT THE FILE NAME ####
write.csv(file="PPPLassoFinal.csv", x=wDF, row.names=FALSE)
